...
const getSalonStats = asyncHandler(async (req, res) => {
    ...
    if (!salon || (role !== 'admin' && !salon.owner.equals(userId))) {
        res.status(403);
        throw new Error('Доступ запрещен. Вы не владелец этого салона.');
    }
    ...
    const masterLoad = await Booking.aggregate([
        { $match: { salon: salon._id, status: 'completed', createdAt: { $gte: weekAgo } } },
    ...
    const masterPerformance = masterLoad.map(item => {
        ...
        return {
            masterName: master ? master.phone : 'Неизвестный',
    ...
    res.status(200).json({
        success: true,
        stats: {
            revenueToday: bookingsToday.length > 0 ? bookingsToday[0].totalRevenue : 0,
            bookingsToday: bookingsToday.length > 0 ? bookingsToday[0].count : 0,
            revenueWeekly: bookingsWeekly.length > 0 ? bookingsWeekly[0].totalRevenue : 0,
...
const getForecasting = asyncHandler(async (req, res) => {
    const bookingCount = await Booking.countDocuments({ salon: req.params.salonId, status: 'completed' });
    const forecastedRevenueNextMonth = bookingCount * 100000;
    const emptyHours = [
        { day: 'Mon', time: '13:00-15:00', suggestion: 'Скидка 15% на стрижку' },
        { day: 'Wed', time: '09:00-11:00', suggestion: 'Счастливые часы, кофе в подарок' },
    ];
    ...
    res.status(200).json({
        success: true,
        forecast: {
            message: 'Это прогноз, основанный на ваших исторических данных (Dummy AI).',
            revenueNextMonth: forecastedRevenueNextMonth,
            growthPercentage: (Math.random() * 10 + 5).toFixed(2), // 5-15% рост
